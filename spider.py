...
# 存储数据的数组
data = []
url = 'http://dl.goufang.com/2s/p1'
# def getData(url):
#   try:
html = requests.get(url).text.decode(
    'utf-8').encode('mbcs')    # 根据url获取页面html并解决编码问题
infos = BeautifulSoup(html, 'lxml')    # 解析html
# titles = infos.select("h3[class='tit'] a")  # 房源名称
# locations = infos.select(
#     "p[class='location'] span[class='community'] a")   # 房源位置
